...
// Draw wire frame box with filled transparent color
void drawRectangle(pcl::visualization::PCLVisualizer::Ptr& viewer, Box box, std::string id, Color color, float opacity) // TODO
{
    if(opacity > 1.0)
        opacity = 1.0;
    if(opacity < 0.0)
...
//    std::string cube = "box"+std::to_string(id);
    //viewer->addCube(box.bboxTransform, box.bboxQuaternion, box.cube_length, box.cube_width, box.cube_height, cube);
    viewer->addCube(box.x_min, box.x_max, box.y_min, box.y_max, box.z_min, box.z_max, color.r, color.g, color.b, id);
    ...
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, color.r, color.g, color.b, id);
